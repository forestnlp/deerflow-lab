"""模型工厂参考件 —— 参考本体 packages/harness/deerflow/models/factory.py。

本体语义：config.yaml 的 models 段每条是 {name, use: "module:attr", ...}，
工厂用 importlib 反射把字符串换成 LangChain ChatModel 类。本件复刻同一语义，
只依赖开源库，并额外提供 --fake 离线模型（同一套 agent 代码可互换）。

用法（各版本复制本件后）：
    from model_factory import get_model, fake_model, tool_call
    model = get_model() if not args.fake else fake_model([...])
"""
...
class FakeToolModel(GenericFakeChatModel):
    """离线脚本模型：bind_tools 直接忽略绑定，按脚本顺序吐消息。"""
    ...
    def _generate(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        if nxt is None:  # 脚本耗尽时优雅收尾，避免循环卡死
