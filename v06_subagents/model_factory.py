"""模型工厂 —— 参考件 ../参考件/model_factory.py 的 v06 自包含副本。

参考本体 packages/harness/deerflow/models/factory.py::create_chat_model
"""
...
def load_config() -> dict:
    return yaml.safe_load(Path(os.environ.get("DFL_CONFIG", CONFIG_PATH)).read_text(encoding="utf-8"))
...
class LatencyFakeToolModel(FakeToolModel):
    """每次吐消息前先睡 latency_s 秒——模拟真实 LLM 的耗时，让并发闸看得见摸得着。"""
...
def fake_model(script: list, latency_s: float = 0.0) -> FakeToolModel:
    """str → 纯文本 AIMessage（循环在此终止）；AIMessage 原样使用。"""
    msgs = [m if isinstance(m, AIMessage) else AIMessage(content=m) for m in script]
    cls = LatencyFakeToolModel if latency_s > 0 else FakeToolModel
    kwargs = {"latency_s": latency_s} if latency_s > 0 else {}
    return cls(messages=iter(msgs), **kwargs)
